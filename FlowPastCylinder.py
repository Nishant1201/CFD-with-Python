...
t = np.linspace(0,100,num=2001,endpoint=True)
dt = t[1] - t[0]
print(dt)
    ...
    while(np.abs(residual) > 0.01):
        ...
        for i in range(1,num_elem_radial-1):
            for j in range(1,num_elem_theta-1):
                ...
                psi[i,j] = (1-beta)*psi0[i,j] + (beta/e[i,j])*( a[i,j]*psi0[i+1,j] + b[i,j]*psi[i-1,j] \
        ...
        for i in range(1,num_elem_radial-1):
            ...
            psi[i,0] = (1-beta)*psi0[i,0] + (beta/e[i,0])*( a[i,0]*psi0[i+1,0] + b[i,0]*psi[i-1,0] \
                            + c[i,0]*psi0[i,1] + d[i,0]*psi[i,-2] + r[i]*omega0[i,0] )
